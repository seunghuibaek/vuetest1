8. 구매_이력 (PrchsHist)        개별 구매 정보
    11  CnsmYn  소비_여부   CHAR(1) Not Null    'Y'  소비 여부가 전체 사용이 됐을때 인지 아니면 일부만 사용되도 소비 여부가 체크 되는지

    1   OrderNo 주문_번호   CHAR(23)    Not Null        PK
    2   PrchsDt 구매_일시   DATETIME(3) Not Null        PK
    3   TrIdnt  트랜잭션_아이디    VARCHAR(128)    Not Null        
    4   SaleGudsRgstCd  판매_상품_등록_코드 CHAR(8) Not Null        FK
    5   Mmbri   회원아이디   INT UNSIGNED    Not Null        FK
    6   StlmtMnCd   결제_수단_코드    MEDIUMINT UNSIGNED  Not Null        FK
    7   StprcWoncrn 결제가격_원화 NUMERIC(19,4)   Not Null    0   
    8   StlmtCrncyCd    결제_통화_코드    CHAR(3) Not Null        FK
    9   StprcAcplcCrncy 결제가격_현지_통화  NUMERIC(19,4)   Not Null    0   
    10  PrchsStat   구매_상태   VARCHAR(25) Not Null        
    11  CnsmYn  소비_여부   CHAR(1) Not Null    'Y'
...
9. 기초_상품 (BsisGuds)
    ...
    1   GudsCd  상품_코드   CHAR(8) Not Null        PK
    2   GudsNm  상품_명    VARCHAR(50) Not Null        
    3   UnitQty 단위_수량   SMALLINT UNSIGNED   Not Null    1   
    4   UnitPrcWoncrn   단위_가격_원화    NUMERIC(19,4)   Not Null    0   
    5   BnsGudsYn   보너스_상품_여부   CHAR(1) Not Null    'Y' 
    6   Descn   설명  VARCHAR(512)            
    7   Rgstdt  등록일시    DATETIME(3) Not Null    CURRENT_TIMESTAMP(3)    
    8   RgstMngri   등록_관리자아이디   VARCHAR(25) Not Null    'Administrator' 
    9   Mdfcdt  수정일시    DATETIME(3)         
    10  MdfcMngri   수정_관리자아이디   VARCHAR(25)         

11. 보너스_상품_지급_이력 (BnsGudsPymntHist)

    1   Mmbri   회원아이디   INT UNSIGNED    Not Null        PFK
    2   OrderNo 주문_번호   CHAR(23)    Not Null        PFK
    3   PrchsDt 구매_일시   DATETIME(3) Not Null        PFK
    4   BnsPymntSeCd    보너스_지급_구분_코드    MEDIUMINT UNSIGNED  Not Null        FK
    5   GudsCd  상품_코드   CHAR(8) Not Null        FK
    6   PymntDd 지급_일자   DATE    Not Null        
    7   PymntQty    지급_수량   INT UNSIGNED    Not Null    0   
    8   ExprtnDd    만료_일자   DATE    Not Null        

33. 판매_상품 (SaleGuds)

    1   SaleGudsCd  판매_상품_코드    CHAR(8) Not Null        PK
    2   BsisGudsCd  기초_상품_코드    CHAR(8)         FK
    3   SaleGudsNm  판매_상품_명 VARCHAR(50) Not Null        
    4   BsisGudsQty 기초_상품_수량    INT UNSIGNED    Not Null    0   
    5   CnvrsnPrcWoncrn 환산_가격_원화    NUMERIC(19,4)   Not Null    0   
    6   MngmGdstpCd 관리_상품유형_코드  MEDIUMINT UNSIGNED          FK
    7   Descn   설명  VARCHAR(512)            
    8   Rgstdt  등록일시    DATETIME(3) Not Null    CURRENT_TIMESTAMP(3)    
    9   RgstMngri   등록_관리자아이디   VARCHAR(25) Not Null    'Administrator' 
    10  Mdfcdt  수정일시    DATETIME(3)         
    11  MdfcMngri   수정_관리자아이디   VARCHAR(25)     

34. 판매_상품_등록_정보 (SaleGudsRgstInfo)

    1   SaleGudsRgstCd  판매_상품_등록_코드 CHAR(8) Not Null        PK
    2   SvcPrvdSeCd 서비스_제공_구분_코드    MEDIUMINT UNSIGNED  Not Null        
    3   OsSeCd  운영체제_구분_코드  MEDIUMINT UNSIGNED  Not Null        
    4   AppMrktCd   애플리케이션_마켓_코드    MEDIUMINT UNSIGNED          FK
    5   InappPrchsGdstpCd   인앱_구매_상품유형_코드   MEDIUMINT UNSIGNED          FK
    6   AppMrktRgstGudsCd   애플리케이션_마켓_등록_상품_코드  VARCHAR(128)            
    7   SaleGudsCd  판매_상품_코드    CHAR(8) Not Null        FK
    8   RgstGudsQty 등록_상품_수량    INT UNSIGNED    Not Null    0   
    9   VatTariff   부가가치세_요율    NUMERIC(5,2)    Not Null        
    10  DscnTariff  할인_요율   NUMERIC(5,2)    Not Null        
    11  PrmiumTariff    할증_요율   NUMERIC(5,2)    Not Null        
    12  SalePrc 판매_가격   NUMERIC(19,4)   Not Null    0   
    13  BnsGudsCd   보너스_상품_코드   CHAR(8) Not Null        FK
    14  BnsQty  보너스_수량  INT UNSIGNED    Not Null    0   
    15  Rgstdt  등록일시    DATETIME(3) Not Null    CURRENT_TIMESTAMP(3)    
    16  RgstMngri   등록_관리자아이디   VARCHAR(25) Not Null    'Administrator' 
    17  Mdfcdt  수정일시    DATETIME(3)         
    18  MdfcMngri   수정_관리자아이디   VARCHAR(25)         

38. 회원_보유_상품 (MmbrHoldGuds)
    1   Mmbri   회원아이디   INT UNSIGNED    Not Null        PFK
    2   BsisGudsCd  기초_상품_코드    CHAR(8) Not Null        PFK
    3   AcmlPymntQty    누적_지급_수량    BIGINT UNSIGNED Not Null    0   
    4   AcmlUseQty  누적_사용_수량    BIGINT UNSIGNED Not Null    0   
    5   AcmlExprtnQty   누적_만료_수량    BIGINT UNSIGNED Not Null    0   
    6   AcmlCnclQty 누적_취소_수량    BIGINT UNSIGNED Not Null    0   
    7   RmndQty 잔여_수량   BIGINT UNSIGNED Not Null    0   
